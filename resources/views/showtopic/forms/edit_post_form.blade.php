...
<div class="topic_textarea bg-white">
    ...
    <div class="tags_container_withkeys">
        <div class="scrolltags">
            <div id="tags_to_save2" style="display:block !important;">
                {!! Post_info($postid, 'answers_to') !!}
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('edit_post', ['postid' => $postid]) }}" method="post">
        ...
        function adderkey() {
            ...
            var x = document.getElementById("tags_to_save2").querySelectorAll("button");
            var i;

                
                for (i = 0; i < x.length; i++) {
                    x[i].innerHTML +='<i class="fas fa-times-circle" id="rb_'+i+'" style="font-size: 16px;" onclick="act_so('+Number(i)+')"></i>';
                }
            

            var countchilds = $('#tags_to_save2').children().length;
            
            if(Number(countchilds) == 0){
                $('#save_answers').val('');
            }
        ...
        function act_so(n) {
            var id = $('#rb_'+Number(n)).closest('button').attr('id');
            $('#'+id).remove();
        }
        ...
        function keyromover(){
            ...
            var i;
            for (i = 0; i < x.length; i++) {
                    if(x[i].removeChild(x[i].lastChild)){
                        $('#save_answers').val($('#tags_to_save2').html());
                    }
            }
            var countchilds = $('#tags_to_save2').children().length;
        ...
        @else
        <textarea name="content" id="summernote" style="display: none !important;">{{ Post_info($postid, 'text') }}</textarea>
        @endif
        ...
        <button type="submit" id="realsubmit" style="display:none !important;">submit</button>
        
        <div id="submitclicker" class="btn btn-primary btn-style mt-4">Post</div>
    </form>
        ...
        {   var raghac = str.split("@")[1].split(" ")[0];
            ...
            $.get("{{ route('user_tags_json') }}?query="+word, function(data, status){
                ...
                if (typeof(fuck) != 'undefined' && fuck != null)
                {
                    ...
                    var the_user_tags_body = '<div id="user_tags_dingdong" class="utsb"></div>';
...
$.insintosummernote = function(id) {
    ...
    function answer(name,to){
        var tc = document.getElementById("tags_to_save2");
        var hm ="array[]";
        var new_chq_no = parseInt($('#total_chq').val())+1;
      
        if(!document.getElementById("new_"+Number(to)) && !document.getElementById("auo"+Number(to))){
  
            var new_input="<input type='text' name="+hm+" id='new_"+Number(to)+"' value="+Number(to)+">";
            var q="'";
            tc.innerHTML += '<button id="auo'+Number(to)+'" class="tag2"><span>'+name+'</span><i class="fas fa-times-circle" style="font-size: 16px;" onclick="rem('+Number(to)+')"></i></button>';
          $('#tags_to_save').append('<button id="auo'+Number(to)+'" class="tag2"><span>@'+name+'</span></button>');
            ...
            if(res){
                ...
                $('#utag_checkd').val('');
                $('#summernote').summernote('pasteHTML', '<span></span>');
            }
        }
    ...
    function rem(w) {
        ...
        const tagvaluetext = $('#auo'+Number(w)).html();
