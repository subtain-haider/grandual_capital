                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    {{ Form::label('name', __('messages.name'), ['class' => 'font-weight-bold login-group__sub-title']) }}
                                    <br>
                                    <span>{{ $role->name ?? 'N/A' }}</span>
                                </div>
                                <div class="form-group col-sm-12">
                                    {{ Form::label('permissions', __('messages.permissions'), ['class' => 'font-weight-bold login-group__sub-title']) }}
                                    <br>
                                    <div class="row col-12">
                                        @if($role->getAllPermissions()->count() > 0)
                                            @foreach($role->getAllPermissions()->pluck('display_name')->toArray() as $permission)
                                                <span class="bg-regent-opacity-3 font-size-3 p-1 mr-4 mb-1 permission-lh">{{ $permission }}</span>
                                            @endforeach
                                        @else
                                            <span class="bg-regent-opacity-3 font-size-3 p-1 mr-4 mb-1 permission-lh">{{ __('messages.N/A') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
