...
$(document).ready(function () {
    ...
    $(document).on('click', '.chat__person-box', function (e) {
        if ($(e.currentTarget).data('id') == selectedContactId) {
            return false;
        }
        callBeforeAPI = false;
    ...
    $(document).on('click', '[data-toggle="lightbox"]', function (event) {
        ...
        $(this).ekkoLightbox();
        setTimeout(function (){
            let ekkoLightBoxModal = $(document).find('.ekko-lightbox.modal.fade.in.show');
            ekkoLightBoxModal.find('div .modal-content').prepend(`<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button></div>`);
        },500);
    });
